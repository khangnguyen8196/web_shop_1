  ...
  tinymce.create("tinymce.util.URI", {
    URI: function (u, s) {
      ...
      if (!/^[\w-]*:?\/\//.test(u)) {
        ...
        u =
          ((s.base_uri && s.base_uri.protocol) || "http") +
          "://mce_host" +
          t.toAbsPath(base_url, u);
      }
    ...
    toAbsolute: function (u, nh) {
      ...
      return u.getURI(
        this.host == u.host && this.protocol == u.protocol ? nh : 0
      );
    },
    ...
    toAbsPath: function (base, path) {
      ...
      // If /a/b/c or /
      if (i <= 0) outPath = o.reverse().join("/");
      else outPath = base.slice(0, i).join("/") + "/" + o.reverse().join("/");
...
(function (tinymce) {
  ...
  tinymce.php = tinymce.php || {};

  tinymce.php.Entities = {
    ...
    getEncodeFunc: function (name, entities) {
      var Entities = tinymce.php.Entities;
...
tinymce.php.Styles = function (settings, schema) {
      ...
      if (css) {
        ...
        while ((matches = styleRegExp.exec(css))) {
          ...
          if (name && value.length > 0) {
            ...
            value = value.replace(
              urlOrStrRegExp,
              function (match, url, url2, url3, str, str2) {
                str = str || str2;
...
(function (tinymce) {
  ...
  function unpack(lookup, data) {
    ...
    replace(data)
      ...
      .replace(
        /(\w+)\[([^\]]+)\]\[([^\]]*)\]/g,
        function (str, name, attributes, children) {
  ...
  tinymce.php.Schema = function (settings) {
    var self = this,
      elements = {},
      children = {},
      patternElements = [],
    ...
    if (!settings.valid_elements) {
      ...
      each(
        split("p,h1,h2,h3,h4,h5,h6,th,td,pre,div,address,caption"),
        function (name) {
  ...
  tinymce.php.Schema.boolAttrMap = boolAttrMap;
  tinymce.php.Schema.blockElementsMap = blockElementsMap;
})(tinymce);

(function (tinymce) {
  tinymce.php.SaxParser = function (settings, schema) {
    var self = this,
      noop = function () {};

    settings = settings || {};
    self.schema = schema = schema || new tinymce.php.Schema();

    if (settings.fix_self_closing !== false) settings.fix_self_closing = true;

    // Add handler functions from settings and setup default handlers
    tinymce.each(
      "comment cdata text start end pi doctype".split(" "),
      function (name) {
    ...
    self.parse = function (html) {
      var self = this,
        ...
        attrValue,
        idCount = 0,
        decode = tinymce.php.Entities.decode,
        fixSelfClosing,
        isIE;
          ...
          if (!validate || (elementRule = schema.getElementRule(value))) {
            ...
            if ((attribsValue = matches[8])) {
              ...
              attribsValue.replace(
                attrRegExp,
                function (match, name, value, val2, val3) {
                  var attrRule, i;
...
(function (tinymce) {
  ...
  tinymce.php.Node = Node;
})(tinymce);

(function (tinymce) {
  var Node = tinymce.php.Node;

  tinymce.php.DomParser = function (settings, schema) {
    var self = this,
      nodeFilters = {},
      attributeFilters = [],
      matchedNodes = {},
    ...
    function fixInvalidChildren(nodes) {
      ...
      nonSplitableElements = tinymce.makeMap(
    ...
    self.filterNode = function (node) {
      ...
      while (i--) {
        ...
        if (name in node.attributes.map) {
          ...
          if (list) list.push(node);
          else matchedAttributes[name] = [node];
        }
      }
    ...
    self.parse = function (html, args) {
      ...
      blockElements = tinymce.extend(
        tinymce.makeMap("script,style,head,html,body,title,meta,param"),
      ...
      parser = new tinymce.php.SaxParser(
        {
          validate: validate,
            ...
            if (elementRule) {
              ...
              while (attrFiltersLen--) {
                ...
                if (attrName in attrs.map) {
                  ...
                  if (list) list.push(newNode);
                  else matchedAttributes[attrName] = [newNode];
                }
              }
            ...
            if (elementRule) {
              if (blockElements[name]) {
                ...
                if (textNode && textNode.type === 3) {
                  ...
                  if (text.length > 0) textNode.value = text;
      ...
      if (validate && invalidChildren.length) {
        if (!args.context) fixInvalidChildren(invalidChildren);
        else args.invalid = true;
      }

      // Wrap nodes in the root into block elements if the root is body
      if (rootBlockName && rootNode.name == "body") addRootBlocks();
      ...
      if (!args.invalid) {
        ...
        for (name in matchedNodes) {
          ...
          while (fi--) {
            if (!nodes[fi].parent) nodes.splice(fi, 1);
          }
          ...
          if (blockElements[node.parent.name] && node === parent.lastChild) {
            ...
            if (node) {
              ...
              if (parent.isEmpty(nonEmptyElements)) {
                ...
                if (elementRule) {
                  if (elementRule.removeEmpty) parent.remove();
                  else if (elementRule.paddEmpty)
                    parent
                      .empty()
                      .append(new tinymce.php.Node("#text", 3)).value =
                      "\u00a0";
                }
              }
            }
...
tinymce.php.Writer = function (settings) {
  var html = [],
    indent,
    indentBefore,
    indentAfter,
  ...
  encode = tinymce.php.Entities.getEncodeFunc(
...
(function (tinymce) {
  tinymce.php.Serializer = function (settings, schema) {
    var self = this,
      writer = new tinymce.php.Writer(settings);
    ...
    self.serialize = function (node) {
      ...
      if (node.type == 1 && !settings.inner) walk(node);
...
(function (tinymce) {
  ...
  var each = tinymce.each,
    ...
    isIE = tinymce.isIE,
    Entities = tinymce.php.Entities,
    simpleSelectorRe = /^([a-z0-9],?)+$/i,
    blockElementsMap = tinymce.php.Schema.blockElementsMap,
    whiteSpaceRegExp = /^[ \t\r\n]*$/;
  ...
  tinymce.create("tinymce.dom.DOMUtils", {
    ...
    DOMUtils: function (d, s) {
      ...
      t.schema = s.schema;
      t.styles = new tinymce.php.Styles(
        {
    ...
    createHTML: function (n, a, h) {
      ...
      if (typeof h != "undefined") return o + ">" + h + "</" + n + ">";
      ...
      return this.run(e, function (e) {
        ...
        switch (n) {
          case "style":
            ...
            if (s.keep_values) {
              if (v && !t._isRes(v)) e.setAttribute("data-mce-style", v, 2);
              else e.removeAttribute("data-mce-style", 2);
            }
    ...
    getAttrib: function (e, n, dv) {
      ...
      if (!e || e.nodeType !== 1) return dv === undef ? false : dv;
      ...
      if (
        ...
        if (e[t.props[n]] === true && v === "") return n;
    ...
    getOuterHTML: function (elm) {
      ...
      if (elm.nodeType === 1 && self.hasOuterHTML) return elm.outerHTML;
    ...
    isBlock: function (node) {
      ...
      if (type) return !!(type === 1 && blockElementsMap[node.nodeName]);
    ...
    toHex: function (s) {
      var c =
        /^\s*rgb\s*?\(\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?,\s*?([0-9]+)\s*?\)\s*$/i.exec(
          s
        );
        ...
        do {
          ...
          if (type === 1) {
            ...
            while (i--) {
              ...
              if (name === "name" || name === "data-mce-bookmark") return false;
            }
          }
      ...
      if (node) {
        for (
          ...
          if (normalized && nodeType == 3) {
            if (nodeType == lastNodeType || !node.nodeValue.length) continue;
          }
          idx++;
    ...
    bind: function (target, name, func, scope) {
      ...
      if (!t.events) t.events = new tinymce.dom.EventUtils();
...
(function (ns) {
  ...
  function Range(dom) {
    ...
    function compareBoundaryPoints(h, r) {
      ...
      if (h === 0) return _compareBoundaryPoints(sc, so, rsc, rso);
    ...
    function insertNode(n) {
      ...
      if (
        ...
        if (o) startContainer.insertBefore(n, o);
        else startContainer.appendChild(n);
      }
    }
    ...
    function _getSelectedNode(container, offset) {
      ...
      if (container.nodeType == 3 /* TEXT_NODE */) return container;
    ...
    function _compareBoundaryPoints(containerA, offsetA, containerB, offsetB) {
      ...
      if (containerA == containerB) {
        if (offsetA == offsetB) return 0; // equal

        if (offsetA < offsetB) return -1; // before
      ...
      if (c) {
        ...
        if (offsetA <= offsetC) return -1; // before
      ...
      if (c) {
        ...
        if (offsetC < offsetB) return -1; // before
      ...
      if (childA == childB) return 0; // equal

      n = cmnRoot.firstChild;
      while (n) {
        if (n == childA) return -1; // before

        if (n == childB) return 1; // after
    ...
    function _traverseSameContainer(how) {
      ...
      if (how != DELETE) frag = doc.createDocumentFragment();
    ...
    function _traverseRightBoundary(root, how) {
      var next = _getSelectedNode(t[END_CONTAINER], t[END_OFFSET] - 1),
        ...
        clonedChild,
        clonedGrandParent,
        isFullySelected = next != t[END_CONTAINER];
    ...
    function _traverseLeftBoundary(root, how) {
      var next = _getSelectedNode(t[START_CONTAINER], t[START_OFFSET]),
        isFullySelected = next != t[START_CONTAINER],
        parent,
        clonedParent,
        nextSibling,
        clonedChild,
    ...
    function _traverseFullySelected(n, how) {
      if (how != DELETE) return how == CLONE ? n.cloneNode(TRUE) : n;
      ...
      if (position < 0) {
        ...
        if (
          ...
          while (parent == checkRng.parentElement()) {
            if (checkRng.move("character", -1) == 0) break;
    ...
    function getRange() {
      ...
      function findEndPoint(start) {
        ...
        if (endPoint.position < 0) {
          ...
          if (!offset) {
            ...
            else domRange[start ? "setStartBefore" : "setEndBefore"](sibling);
          ...
          if (!offset) {
            ...
            else domRange[start ? "setStartAfter" : "setEndAfter"](sibling);
      ...
      try {
        ...
        if (ex.number == -2147024809) {
          ...
          // Find end point if needed
          if (!collapsed) findEndPoint();
        } else throw ex; // Throw other errors
      }
...
(function (tinymce) {
  ...
  tinymce.create("tinymce.dom.Selection", {
    ...
    getContent: function (s) {
      ...
      } else if (is(r.item) || is(r.phpText)) {
        // IE will produce invalid markup if elements are present that
        // it doesn't understand like custom elements or HTML5 elements.
        // Adding a BR in front of the contents and then remoiving it seems to fix it though.
        e.innerHTML = "<br>" + (r.item ? r.item(0).outerHTML : r.phpText);
...
(function (tinymce) {
  tinymce.dom.Serializer = function (settings, dom, schema) {
    ...
    dom = dom || tinymce.DOM;
    schema = schema || new tinymce.php.Schema(settings);
    ...
    htmlParser = new tinymce.php.DomParser(settings, schema);
    ...
    htmlParser.addNodeFilter("script,style", function (nodes, name) {
      ...
      while (i--) {
        ...
        if (name === "script") {
          ...
          node.attr(
            "type",
            (node.attr("type") || "text/javascript").replace(/^mce\-/, "")
          );
    ...
    return {
      ...
      serialize: function (node, args) {
        ...
        if (impl.createHTMLDocument) {
          ...
          each(
            node.nodeName == "BODY" ? node.childNodes : [node],
            function (node) {
          ...
          if (node.nodeName != "BODY") node = doc.body.firstChild;
          else node = doc.body;
        ...
        // Setup serializer
        htmlSerializer = new tinymce.php.Serializer(settings, schema);
        ...
        args.content = htmlSerializer.serialize(
          htmlParser.parse(
            args.getInner
              ? node.innerHTML
              : tinymce.trim(dom.getOuterHTML(node), args),
            args
          )
        );
...
(function (tinymce) {
  tinymce.dom.RangeUtils = function (dom) {
    ...
    this.walk = function (rng, callback) {
      ...
      if (endContainer.nodeType == 1 && endContainer.hasChildNodes())
        endContainer =
          endContainer.childNodes[
            Math.min(endOffset - 1, endContainer.childNodes.length - 1)
          ];
      ...
      // Same container
      if (startContainer == endContainer) return callback([startContainer]);
      ...
      if (siblings.length) callback(siblings);
  ...
  tinymce.create("tinymce.ui.ListBox:tinymce.ui.Control", {
    ...
    renderMenu: function () {
      ...
      m.add({
        ...
        onclick: function () {
          if (t.settings.onselect("") !== false) t.select(""); // Must be runned after
        },
      });
      ...
      each(t.items, function (o) {
        ...
        if (o.value === undefined) {
          ...
          o.role = "option";
          o.onclick = function () {
            if (t.settings.onselect(o.value) !== false) t.select(o.value); // Must be runned after
          };
...
(function (tinymce) {
  ...
  tinymce.create("tinymce.ui.NativeListBox:tinymce.ui.ListBox", {
    ...
    renderHTML: function () {
      ...
      h += DOM.createHTML(
        "span",
        { id: t.id + "_aria", style: "display: none" },
        t.settings.title
      );
      return h;
    },
...
(function (tinymce) {
  ...
  tinymce.create("tinymce.ui.ColorSplitButton:tinymce.ui.SplitButton", {
    ...
    renderMenu: function () {
      ...
      i = 0;
      each(
        is(s.colors, "array") ? s.colors : s.colors.split(","),
        function (c) {
          c = c.replace(/^#/, "");
      ...
      Event.add(t.id + "_menu", "click", function (e) {
        ...
        if (
          ...
          (c = e.getAttribute("data-mce-color"))
        )
          t.setColor(c);
    ...
    displayColor: function (c) {
      ...
      DOM.setStyle(t.id + "_detail", "backgroundColor", c);
    ...
    postRender: function () {
      ...
      DOM.add(id + "_action", "div", {
        id: id + "_detail",
        class: "mceColordetail",
      });
      DOM.setStyle(t.id + "_detail", "backgroundColor", t.value);
    },
...
(function (tinymce) {
  ...
  tinymce.create("tinymce.AddOnManager", {
    ...
    load: function (n, u, cb, s) {
      ...
      if (t.urls[n]) return;
      if (typeof u === "object") url = u.prefix + u.resource + u.suffix;
...
(function (tinymce) {
  ...
  tinymce.documentBaseURL = window.location.href
    .replace(/[\?#].*$/, "")
    .replace(/[\/\\][^\/]+$/, "");
  if (!/[\/\\]$/.test(tinymce.documentBaseURL)) tinymce.documentBaseURL += "/";

  tinymce.baseURL = new tinymce.util.URI(tinymce.documentBaseURL).toAbsolute(
    tinymce.baseURL
  );
  ...
  tinymce.EditorManager = extend(tinymce, {
    ...
    get: function (id) {
      if (id === undefined) return this.editors;
...
(function (tinymce) {
  ...
  tinymce.create("tinymce.Editor", {
    ...
    init: function () {
      ...
      s.aria_label =
        s.aria_label ||
        DOM.getAttrib(e, "aria-label", t.getLang("aria.rich_text_area"));
      ...
      t.iframeHTML =
        s.doctype + '<html><head xmlns="http://www.w3.org/1999/xhtml">';
      ...
      if (
        ...
        (isIE || (tinymce.isOpera && parseFloat(opera.version()) < 11))
      ) {
      ...
      if (!tinymce.relaxedDomain || !u) t.setupIframe();
    ...
    setupIframe: function () {
      ...
      if (!isIE || !tinymce.relaxedDomain) {
        ...
        if (tinymce.relaxedDomain) d.domain = tinymce.relaxedDomain;
      }
      ...
      t.schema = new tinymce.php.Schema(s);
      ...
      t.parser = new tinymce.php.DomParser(s, t.schema);
      ...
      t.parser.addAttributeFilter("src,href,style", function (nodes, name) {
        ...
        while (i--) {
          ...
          if (!node.attributes.map[internalName]) {
            ...
            else node.attr(internalName, t.convertURL(value, name, node.name));
          }
        }
      });
      ...
      t.parser.addNodeFilter("script", function (nodes, name) {
        ...
        while (i--) nodes[i].attr("type", "mce-text/javascript");
      });
      ...
      t.parser.addNodeFilter("p,h1,h2,h3,h4,h5,h6,div", function (nodes, name) {
        var i = nodes.length,
          node,
          nonEmptyElements = t.schema.getNonEmptyElements();
        ...
        while (i--) {
          ...
          if (node.isEmpty(nonEmptyElements))
            node
              .empty()
              .append(new tinymce.php.Node("br", 1)).shortEnded = true;
        }
      });
      ...
      t.undoManager.onAdd.add(function (um, l) {
        if (um.hasUndo()) return t.onChange.dispatch(t, l, um);
      });
    ...
    getLang: function (n, dv) {
      return (
        tinymce.i18n[(this.settings.language || "en") + "." + n] ||
        (is(dv) ? dv : "{#" + n + "}")
      );
    },
    ...
    setContent: function (content, args) {
      ...
      if (!tinymce.isIE && (content.length === 0 || /^\s+$/.test(content))) {
        ...
        if (forcedRootBlockName)
          content =
            ...
            forcedRootBlockName +
            ">";
        else content = '<br data-mce-bogus="1">';
      ...
      if (args.format !== "raw") {
        content = new tinymce.php.Serializer({}, self.schema).serialize(
    ...
    getContent: function (args) {
      ...
      if (args.format == "raw") content = self.getBody().innerHTML;
      else content = self.serializer.serialize(self.getBody(), args);
    ...
    isDirty: function () {
      ...
      return (
        tinymce.trim(self.startContent) !=
          tinymce.trim(self.getContent({ format: "raw", no_events: 1 })) &&
        !self.isNotDirty
      );
    },
    ...
    _addEvents: function () {
      ...
      t.onKeyDown.add(function (ed, e) {
        ...
        if (
          n &&
          n.parentNode &&
          n.parentNode.tagName === "BLOCKQUOTE" &&
      ...
      if (tinymce.isIE) {
        ...
        dom.bind(t.getDoc(), "controlselect", function (e) {
          ...
          if (re) dom.unbind(re.node, re.ev, re.cb);
      ...
      if (s.custom_undo_redo) {
        ...
        dom.bind(t.getDoc(), "focusout", function (e) {
          if (!t.removed && t.undoManager.typing) addUndo();
        });
        ...
        t.onKeyDown.add(function (ed, e) {
          ...
          if (
            ...
            if (tinymce.isIE && keyCode == 13) t.undoManager.beforeChange();

            if (t.undoManager.typing) addUndo();
      ...
      if (tinymce.isGecko) {
        function getAttributeApplyFunction() {
          var template = t.dom.getAttribs(
            t.selection.getStart().cloneNode(false)
          );
    ...
    addCommands({
        ...
        if (failed || !doc.queryCommandSupported(command)) {
          if (tinymce.isGecko) {
            editor.windowManager.confirm(
              editor.getLang("clipboard_msg"),
              function (state) {
                if (state)
                  open(
                    "http://www.mozilla.org/editor/midasdemo/securityprefs.php",
                    "_blank"
                  );
              }
            );
      ...
      FontSize: function (command, ui, value) {
        ...
        if (value >= 1 && value <= 7) {
          ...
          if (fontClasses) value = fontClasses[value - 1] || value;
          else value = fontSizes[value - 1] || value;
        }
      ...
      mceInsertContent: function (command, ui, value) {
        ...
        if (!args.invalid) {
          ...
          else selection.setContent(value);
        } else {
          ...
          if (parentNode.nodeType == 9) parentNode = node = rootNode;
          ...
          if (parentNode == rootNode) dom.setHTML(rootNode, value);
          else dom.setOuterHTML(parentNode, value);
        }
        ...
        if (
          nodeRect.y + nodeRect.h > viewPortRect.y + viewPortRect.h ||
          nodeRect.y < viewPortRect.y ||
          nodeRect.x > viewPortRect.x + viewPortRect.w ||
          nodeRect.x < viewPortRect.x
        ) {
          viewportBodyElement = tinymce.isIE
            ? editor.getDoc().documentElement
            : editor.getBody();
    ...
    addCommands(
      {
        "FontSize,FontName": function (command) {
          ...
          if ((parent = dom.getParent(selection.getNode(), "span"))) {
            if (command == "fontsize") value = parent.style.fontSize;
            else
...
(function (tinymce) {
  tinymce.Formatter = function (ed) {
    ...
    function isCaretNode(node) {
      return (
        node.nodeType === 1 &&
        (node.face === "mceinline" || node.style.fontFamily === "mceinline")
      );
    }
      ...
      if (name) {
        if (typeof name !== "string") {
          ...
          each(format, function (format) {
            ...
            if (format.deep === undefined) format.deep = !format.selector;
    ...
    function apply(name, vars, node) {
      ...
      function applyStyleToList(node, bookmark, wrapElm, newWrappers, process) {
        ...
        if (
          listIndex <= 0 ||
          (startIndex < listIndex && endIndex > listIndex)
        ) {
          ...
          each(tinymce.grep(node.childNodes), function (n, index) {
            if (
              (startIndex < listIndex && index < listIndex) ||
              (startIndex > listIndex && index > listIndex)
            ) {
              nodes.push(n);
              n.parentNode.removeChild(n);
      ...
      function applyRngStyle(rng, bookmark) {
        ...
        rangeUtils.walk(rng, function (nodes) {
          ...
          function process(node) {
            ...
            if (
              ...
              currentWrapElm = applyStyleToList(
                node,
                bookmark,
                wrapElm,
                newWrappers,
        ...
        each(newWrappers, function (node) {
          ...
          if (format.inline || format.wrapper) {
            ...
            if (!format.exact && childCount === 1) node = mergeStyles(node);
    ...
    function remove(name, vars, node) {
      ...
      function moveStart(rng) {
        ...
        if (container.nodeType == 1) {
          ...
          if (offset > nodes.length - 1) walker.next();
      ...
      function process(node) {
        ...
        for (i = 0, l = formatList.length; i < l; i++) {
          if (removeFormat(formatList[i], vars, node, node)) break;
        }
      ...
      function findFormatRoot(container) {
        ...
        each(getParents(container.parentNode).reverse(), function (parent) {
          ...
          if (!formatRoot && parent.id != "_start" && parent.id != "_end") {
            ...
            if (format && format.split !== false) formatRoot = parent;
          }
        });
          ...
          if (
            ...
            if ((classes = format.classes)) {
              for (i = 0; i < classes.length; i++) {
                if (!dom.hasClass(node, classes[i])) return;
              }
            }
    ...
    function match(name, vars, node) {
      ...
      if (selection.isCollapsed()) {
        for (i = pendingFormats.apply.length - 1; i >= 0; i--) {
          if (pendingFormats.apply[i].name == name) return true;
        }

        for (i = pendingFormats.remove.length - 1; i >= 0; i--) {
          if (pendingFormats.remove[i].name == name) return false;
        }
      ...
      if (startNode != node) {
        if (matchParents(startNode)) return TRUE;
      }
      ...
      if (formatList) {
        ...
        for (x = formatList.length - 1; x >= 0; x--) {
          ...
          for (i = parents.length - 1; i >= 0; i--) {
            if (dom.is(parents[i], selector)) return TRUE;
          }
        }
      }
    ...
    function replaceVars(value, vars) {
      if (typeof value != "string") value = value(vars);
      else if (vars) {
    ...
    function removeFormat(format, vars, node, compare_node) {
      ...
      if (format.remove != "all") {
        ...
        for (i = 0; i < attrs.length; i++) {
          if (attrs[i].nodeName.indexOf("_") !== 0) return FALSE;
        }
      }
            ...
            if (!format.list_block || !isEq(node, format.list_block)) {
              each(tinymce.grep(node.childNodes), function (node) {
                if (isValid(forcedRootBlock, node.nodeName.toLowerCase())) {
                  if (!rootBlockElm) rootBlockElm = wrap(node, forcedRootBlock);
                  else rootBlockElm.appendChild(node);
                } else rootBlockElm = 0;
              });
            }
          }
    ...
    function getNonWhiteSpaceSibling(node, next, inc) {
      if (node) {
        ...
        for (node = inc ? node : node[next]; node; node = node[next]) {
          if (node.nodeType == 1 || !isWhiteSpaceNode(node)) return node;
        }
      }
    }
      ...
      function compareElements(node1, node2) {
        ...
        function compareObjects(obj1, obj2) {
          ...
          for (name in obj2) {
            ...
            if (obj2.hasOwnProperty(name)) return FALSE;
          }
    ...
    function performCaretAction(type, name, vars) {
      var i,
        ...
        otherPendingFormats =
          pendingFormats[type == "apply" ? "remove" : "apply"];
      ...
      for (i = currentPendingFormats.length - 1; i >= 0; i--) {
        if (currentPendingFormats[i].name == name) return;
      }
          ...
          each(
            ...
            function (event) {
              ed[event].addToTop(function (ed, e) {
                ...
                if (
                  hasPending() &&
                  !tinymce.dom.RangeUtils.compareRanges(
                    pendingFormats.lastRng,
                    selection.getRng()
                  )
                ) {
